...
void getFromDB(char *userid, char *passcode){
	...
	if (sqlite3_step(stmt) == SQLITE_ROW) {
		//temp = {sqlite3_column_text(stmt, 0), sqlite3_column_text(stmt, 1), sqlite3_column_text(stmt, 2), sqlite3_column_text(stmt, 3), sqlite3_column_text(stmt, 4)};
...
void getPassDB(char *userid, char* output){
	...
	if (sqlite3_step(stmt) == SQLITE_ROW) {
		...
		//char *output =  malloc(strlen(name) * sizeof(char));
		strcpy(output,name);
		printf("\n Passcode: %s \n", output);
 	}else{
		printf("\nCould not step (execute) stmt.\n");
