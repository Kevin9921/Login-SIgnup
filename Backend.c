...
//Jim@1234
	...
	free(DBPasscode);
	free(UserPasscode);
	//char *encrpyt = malloc((strlen(DBPasscode)) * sizeof(char));
	//const unsigned char* temp = getSaltDB(userid, "Password")
...
int passwordMatch(char *Password){
	...
	char words[3][10] = {	"Password1",
				"Qwerty123",
				"Qaz123wax"
	};
	for (int i =0; i<3; i++){
		if (!strcmp(Password,words[i])){
			return 0;		
		}
	}
	return 1;
